(() => {
  ...
  // web/src/themes.js
  function detectProfile(folderName) {
    const raw = String(folderName || "").trim();
    const cleaned = raw.replace(/portfolio/ig, "").trim();
    const slug2 = cleaned.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
    if (THEMES[slug2]) return { id: slug2, theme: THEMES[slug2] };
    for (const [id, t2] of Object.entries(THEMES)) {
      const nameSlug = t2.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
      if (nameSlug === slug2 || slug2.startsWith(nameSlug) || nameSlug.startsWith(slug2)) return { id, theme: t2 };
    }
    const name = cleaned || "Studio";
    return { id: slug2 || "studio", theme: { ...DEFAULT, id: slug2 || "studio", name } };
  }
  var THEMES, DEFAULT;
  ...
  function parseCSV(text2) {
    ...
    while (rows.length && rows[rows.length - 1].every((x2) => x2.trim() === "")) rows.pop();
    if (!rows.length) return { header: [], records: [] };
    const header = rows[0].map((h2) => h2.trim());
    const records = rows.slice(1).filter((r2) => r2.some((x2) => x2.trim() !== "")).map((r2) => {
      const o2 = {};
      header.forEach((h2, idx) => {
        o2[h2] = (r2[idx] ?? "").trim();
      });
      return o2;
    });
  ...
  // web/src/portfolio.js
  function parseName(name, typeKey) {
    const parts = name.split(/\s*-\s*/).map((s2) => s2.trim()).filter(Boolean);
    ...
    if (hasDesc) {
      if (parts.length >= 3) {
        ...
        out.descriptor = parts.slice(2).join(" ");
      } else {
        const p2 = parts[1];
        if (typeKey === "logos" && isLogoDesc(p2) || typeKey === "guidelines" && isGuideDesc(p2)) out.descriptor = p2;
        else out.industry = p2;
      }
    } else {
  ...
  function parseTags(text2) {
    const map = /* @__PURE__ */ new Map();
    if (!text2) return map;
    const { records } = parseCSV(text2);
    for (const r2 of records) {
      const key = (r2.brand || r2.name || (r2.file || "").replace(/\.[^.]+$/, "")).trim().toLowerCase();
      if (!key) continue;
      const split = (s2) => (s2 || "").split("|").map(norm).filter(Boolean);
      map.set(key, { industries: split(r2.industries), types: split(r2.types) });
    }
    return map;
  }
  function kindOf(ext) {
  ...
  function classify(files, typeKey) {
    return files.filter((f2) => ALL_EXT.includes(extOf(f2))).map((f2) => {
      const base = f2.replace(/\.[^.]+$/, "").trim();
      const p2 = parseName(base, typeKey);
      return {
        brand: p2.brand,
        file: f2,
        ext: extOf(f2),
        kind: kindOf(extOf(f2)),
        _industry: p2.industry ? slug(p2.industry) : "",
        _descriptor: p2.descriptor
      };
    }).sort((a2, b2) => a2.brand.localeCompare(b2.brand));
  }
  function attachMeta(list, typeKey, tags) {
    for (const a2 of list) {
      const t2 = tags && tags.get(a2.brand.toLowerCase()) || {};
      a2.industries = uniq([...a2._industry ? [a2._industry] : [], ...t2.industries || []]);
      if (typeKey === "logos")
        a2.types = uniq([...a2._descriptor ? [mapLogoType(a2._descriptor)] : [], ...t2.types || []]);
      a2.industryLabel = a2._industry ? a2._industry.replace(/-/g, " ") : "";
      a2.descriptor = a2._descriptor || "";
      delete a2._industry;
      delete a2._descriptor;
    }
    return list;
  }
  var norm, slug, uniq, LOGO_TYPE_MAP, mapLogoType, isLogoDesc, isGuideDesc, TYPES, IMAGE_EXT, PDF_EXT, VIDEO_EXT, GIF_EXT, ALL_EXT, extOf, FsPortfolio, DemoPortfolio;
  var init_portfolio = __esm({
    "web/src/portfolio.js"() {
      init_themes();
      init_csv();
      norm = (v2) => v2.trim().toLowerCase().replace(/\s+/g, "-");
      slug = (v2) => String(v2).trim().toLowerCase().replace(/&/g, " and ").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
      uniq = (a2) => [...new Set(a2.filter(Boolean))];
      LOGO_TYPE_MAP = {
        logotype: "wordmark",
        wordmark: "wordmark",
        "word-mark": "wordmark",
        ...
        "letter-mark": "lettermark",
        monogram: "lettermark",
        initial: "lettermark",
        initials: "lettermark",
        pictorial: "pictorial",
        "pictorial-mark": "pictorial",
        brandmark: "pictorial",
      ...
      mapLogoType = (d2) => {
        const s2 = slug(d2);
        return LOGO_TYPE_MAP[s2] || s2;
      };
      isLogoDesc = (d2) => {
        const s2 = slug(d2);
        return s2 in LOGO_TYPE_MAP || /(^|-)(mark|type)$/.test(s2);
      };
      isGuideDesc = (d2) => /guide|guideline|brand-?book|style/.test(slug(d2));
      TYPES = [
        {
          key: "logos",
          label: "Logos",
        ...
        },
        {
          ...
          dirs: ["brand guidelines", "guidelines", "brand-guidelines"],
          strip: ["brand guidelines", "guidelines", "brand guide", "guide"]
        },
        {
          key: "social",
          label: "Social Media Kit",
          dirs: ["social media kit", "social media", "social-media-kit", "social"],
          strip: ["social media kit", "social media", "social kit", "social"]
        },
        {
          key: "stationery",
          label: "Stationery",
          dirs: ["stationery", "stationary"],
          strip: ["stationery", "stationary"]
        },
        {
          key: "animation",
          label: "Logo Animation",
          dirs: ["logo animation", "logo animations", "logo-animation", "animation", "animations"],
      ...
      GIF_EXT = ["gif"];
      ALL_EXT = [...IMAGE_EXT, ...PDF_EXT, ...VIDEO_EXT, ...GIF_EXT];
      extOf = (f2) => (f2.split(".").pop() || "").toLowerCase();
      FsPortfolio = class {
        ...
        profile() {
          return detectProfile(this.root.name);
        }
        async _typeDirs() {
          if (this._dirs) return this._dirs;
          const map = {};
          for await (const [name, h2] of this.root.entries()) {
            if (h2.kind !== "directory") continue;
            const low = name.toLowerCase().trim();
            const t2 = TYPES.find((x2) => x2.dirs.includes(low));
            if (t2 && !map[t2.key]) map[t2.key] = h2;
          }
          this._dirs = map;
          return map;
        }
        async types() {
          const d2 = await this._typeDirs();
          return TYPES.filter((t2) => d2[t2.key]);
        }
        async _tags() {
          if (this._tagMap) return this._tagMap;
          let text2 = null;
          try {
            text2 = await (await (await this.root.getFileHandle("tags.csv")).getFile()).text();
          } catch {
          }
          if (!text2) {
            try {
              const d2 = await this._typeDirs();
              if (d2.logos) text2 = await (await (await d2.logos.getFileHandle("tags.csv")).getFile()).text();
            } catch {
            }
          }
          this._tagMap = parseTags(text2);
          return this._tagMap;
        }
        async assets(typeKey) {
          const d2 = await this._typeDirs();
          if (!d2[typeKey]) return [];
          const files = [];
          for await (const [fname, h2] of d2[typeKey].entries()) if (h2.kind === "file" && fname.toLowerCase() !== "tags.csv") files.push(fname);
          const list = classify(files, typeKey);
          return attachMeta(list, typeKey, await this._tags());
        }
        async hasTags() {
          return (await this._tags()).size > 0;
        }
        async getBlob(typeKey, file) {
          const d2 = await this._typeDirs();
          const fh = await d2[typeKey].getFileHandle(file);
          return fh.getFile();
        }
      };
      DemoPortfolio = class {
        constructor(base = "demo/portfolio") {
          this.base = base;
          this._m = null;
        }
        async _manifest() {
          if (!this._m) {
            try {
              ...
              console.warn("Demo portfolio not available on file:// protocol:", e2);
        ...
        async types() {
          const m2 = await this._manifest();
          return TYPES.filter((t2) => (m2.types[t2.key] || []).length);
        }
        async assets(typeKey) {
          const m2 = await this._manifest();
          const list = classify(m2.types[typeKey] || [], typeKey);
          const tags = new Map(Object.entries(m2.tags || {}).map(([k2, v2]) => [
            k2.toLowerCase(),
            { industries: (v2.industries || []).map(norm), types: (v2.types || []).map(norm) }
          ]));
        ...
        async getBlob(typeKey, file) {
          const m2 = await this._manifest();
          const folder = m2.dirs && m2.dirs[typeKey] || TYPES.find((x2) => x2.key === typeKey).label;
  ...
  // web/src/vocab.js
  var INDUSTRIES, TYPES2, INDUSTRY_SET, TYPE_SET;
  var init_vocab = __esm({
    "web/src/vocab.js"() {
      ...
      INDUSTRY_SET = new Set(INDUSTRIES);
      TYPE_SET = new Set(TYPES2);
    }
  });
  ...
  // web/src/curate.js
  function describeQuery(industries, types, matchAll) {
    const parts = [];
  ...
  function titleCase(s2) {
    return s2.replace(/-/g, " ").replace(/\b\w/g, (c2) => c2.toUpperCase());
  }
  ...
  function drawTriangle(t2, e2, i2, s2, n2, a2, r2, o2) {
    ...
    if (g2 >= A2) return;
    const w2 = h2[a2], v2 = h2[a2 + 1], y2 = h2[a2 + 2], x2 = h2[r2], _2 = h2[r2 + 1], E2 = h2[r2 + 2], S2 = h2[o2], C2 = h2[o2 + 1], T2 = h2[o2 + 2], M2 = Math.round(g2), P3 = Math.round(A2);
    let D2, k2, R2, I2, F2, L2, O2, N2;
    for (let t3 = M2; t3 <= P3; t3++) {
      if (t3 < f2) {
  ...
  function getDocument(t2 = {}) {
    ...
    c2.promise.then((function() {
      ...
      let l3;
      if (h2) l3 = new PDFDataTransportStream(h2, { disableRange: P3, disableStream: D2 });
      else if (!a2) {
        ...
        l3 = new t4({ url: n2, length: O2, httpHeaders: r2, withCredentials: o2, rangeChunkSize: d2, disableRange: P3, disableStream: D2 });
      }
      return t3.then(((t4) => {
        if (i2.destroyed) throw new Error("Loading aborted");
        if (c2.destroyed) throw new Error("Worker was destroyed");
  ...
  var import_meta, t, __webpack_exports__, e, i, s, n, a, r, o, l, h, d, c, u, p, g, m, f, b, A, w, v, y, x, _, E, S, C, T, M, P, D, k, R, I, F, L, O, N, B, H, z, U, G, $, V, j, W, q, X, K, Y, Q, PasswordException, UnknownErrorException, InvalidPDFException, MissingPDFException, UnexpectedResponseException, FormatError, AbortException, util_FeatureTest, J, Util, Z, tt, et, it, PixelsPerInch, PageViewport, RenderingCancelledException, StatTimer, PDFDateString, OutputScale, EditorToolbar, HighlightToolbar, IdManager, ImageManager, CommandManager, KeyboardManager, ColorManager, AnnotationEditorUIManager, AltText, TouchManager, AnnotationEditor, FakeEditor, st, nt, at, MurmurHash3_64, rt, AnnotationStorage, PrintAnnotationStorage, FontLoader, FontFaceObject, ot, lt, ht, dt, ct, ut, pt, gt, mt, ft, MessageHandler, BaseCanvasFactory, BaseCMapReaderFactory, DOMCMapReaderFactory, BaseFilterFactory, BaseStandardFontDataFactory, DOMStandardFontDataFactory, bt, At, wt, BaseShadingPattern, RadialAxialShadingPattern, MeshShadingPattern, DummyShadingPattern, vt, yt, TilingPattern, xt, CachedCanvases, CanvasExtraState, _t, Et, St, Ct, CanvasGraphics, GlobalWorkerOptions, Metadata, Tt, OptionalContentGroup, OptionalContentConfig, PDFDataTransportStream, PDFDataTransportStreamReader, PDFDataTransportStreamRangeReader, PDFFetchStream, PDFFetchStreamReader, PDFFetchStreamRangeReader, NetworkManager, PDFNetworkStream, PDFNetworkStreamFullRequestReader, PDFNetworkStreamRangeRequestReader, Mt, PDFNodeStream, PDFNodeStreamFsFullReader, PDFNodeStreamFsRangeReader, Pt, TextLayer, XfaText, Dt, kt, Rt, It, Ft, PDFDocumentLoadingTask, PDFDataRangeTransport, PDFDocumentProxy, PDFPageProxy, LoopbackPort, PDFWorker, WorkerTransport, Lt, PDFObjects, RenderTask, InternalRenderTask, Ot, Nt, ColorConverters, BaseSVGFactory, DOMSVGFactory, XfaLayer, Bt, Ht, AnnotationElementFactory, AnnotationElement, LinkAnnotationElement, TextAnnotationElement, WidgetAnnotationElement, TextWidgetAnnotationElement, SignatureWidgetAnnotationElement, CheckboxWidgetAnnotationElement, RadioButtonWidgetAnnotationElement, PushButtonWidgetAnnotationElement, ChoiceWidgetAnnotationElement, PopupAnnotationElement, PopupElement, FreeTextAnnotationElement, LineAnnotationElement, SquareAnnotationElement, CircleAnnotationElement, PolylineAnnotationElement, PolygonAnnotationElement, CaretAnnotationElement, InkAnnotationElement, HighlightAnnotationElement, UnderlineAnnotationElement, SquigglyAnnotationElement, StrikeOutAnnotationElement, StampAnnotationElement, FileAttachmentAnnotationElement, AnnotationLayer, zt, FreeTextEditor, Outline, FreeDrawOutliner, FreeDrawOutline, HighlightOutliner, HighlightOutline, FreeHighlightOutliner, FreeHighlightOutline, ColorPicker, HighlightEditor, DrawingOptions, DrawingEditor, InkDrawOutliner, InkDrawOutline, InkDrawingOptions, InkEditor, StampEditor, AnnotationEditorLayer, DrawLayer, Ut, Gt, $t, Vt, jt, Wt, qt, Xt, Kt, Yt, Qt, Jt, Zt, te, ee, ie, se, ne, ae, re, oe, le, he, de, ce, ue, pe, ge, me, fe, be, Ae, we, ve, ye, xe, _e, Ee, Se, Ce, Te, Me, Pe, De, ke;
  var init_pdf_min = __esm({
    "web/lib/pdf.min.js"() {
      import_meta = {};
      t = { d: (e2, i2) => {
      ...
      m = { RESIZE: 1, CREATE: 2, FREETEXT_SIZE: 11, FREETEXT_COLOR: 12, FREETEXT_OPACITY: 13, INK_COLOR: 21, INK_THICKNESS: 22, INK_OPACITY: 23, HIGHLIGHT_COLOR: 31, HIGHLIGHT_DEFAULT_COLOR: 32, HIGHLIGHT_THICKNESS: 33, HIGHLIGHT_FREE: 34, HIGHLIGHT_SHOW_ALL: 35, DRAW_STEP: 41 };
      ...
      Y = q.WARNINGS;
      Q = (function BaseExceptionClosure() {
      ...
      PasswordException = class extends Q {
        constructor(t2, e2) {
          super(t2, "PasswordException");
          this.code = e2;
        }
      };
      UnknownErrorException = class extends Q {
        constructor(t2, e2) {
          super(t2, "UnknownErrorException");
          this.details = e2;
        }
      };
      InvalidPDFException = class extends Q {
        constructor(t2) {
          super(t2, "InvalidPDFException");
        }
      };
      MissingPDFException = class extends Q {
        constructor(t2) {
          super(t2, "MissingPDFException");
        }
      };
      UnexpectedResponseException = class extends Q {
        constructor(t2, e2) {
          super(t2, "UnexpectedResponseException");
          this.status = e2;
        }
      };
      FormatError = class extends Q {
        constructor(t2) {
          super(t2, "FormatError");
        }
      };
      AbortException = class extends Q {
        constructor(t2) {
      ...
      J = Array.from(Array(256).keys(), ((t2) => t2.toString(16).padStart(2, "0")));
      Util = class {
      ...
      RenderingCancelledException = class extends Q {
        constructor(t2, e2 = 0) {
...
`);
      ...
      rt = Object.freeze({ map: null, hash: "", transfer: void 0 });
      ...
      TilingPattern = class _TilingPattern {
        ...
        createPatternCanvas(t2) {
          ...
          const S2 = this.getSizeAndScale(w2, this.ctx.canvas.width, b2), C2 = this.getSizeAndScale(v2, this.ctx.canvas.height, A2), T2 = t2.cachedCanvases.getCanvas("pattern", S2.size, C2.size), M2 = T2.context, P3 = r2.createCanvasGraphics(M2);
          P3.groupLevel = t2.groupLevel;
          this.setFillAndStrokeStyleToContext(P3, s2, a2);
          M2.translate(-S2.scale * h2, -C2.scale * d2);
          P3.transform(S2.scale, 0, 0, C2.scale, 0, 0);
          M2.save();
          this.clipBbox(P3, h2, d2, c2, u2);
          P3.baseTransform = getCurrentTransform(P3.ctx);
          P3.executeOperatorList(i2);
          P3.endDrawing();
          M2.restore();
          if (y2 || x2) {
            const e3 = T2.canvas;
      ...
      Dt = 65536;
      kt = e ? class NodeCanvasFactory extends BaseCanvasFactory {
        _createCanvas(t2, e2) {
      ...
      Rt = e ? class NodeCMapReaderFactory extends BaseCMapReaderFactory {
        async _fetch(t2) {
          return node_utils_fetchData(t2);
        }
      } : DOMCMapReaderFactory;
      It = e ? class NodeFilterFactory extends BaseFilterFactory {
      ...
      Ft = e ? class NodeStandardFontDataFactory extends BaseStandardFontDataFactory {
        async _fetch(t2) {
          return node_utils_fetchData(t2);
        }
      ...
      Lt = /* @__PURE__ */ Symbol("INITIAL_DATA");
      PDFObjects = class {
      ...
      InkDrawOutline = class _InkDrawOutline extends Outline {
        #pa;
        #Lr = 0;
        #Wn;
        #Pr;
      ...
      Ut = __webpack_exports__.AbortException;
  ...
  function loadPdfjs() {
    if (!_pdfjs) {
      _pdfjs = Promise.resolve().then(() => (init_pdf_min(), pdf_min_exports)).then((m2) => {
        m2.GlobalWorkerOptions.workerSrc = "lib/pdf.worker.min.js";
        return m2;
      });
    }
    return _pdfjs;
  ...
  async function renderSvg(blob, maxEdge) {
    ...
    try {
      ...
      let w2 = img.naturalWidth, h2 = img.naturalHeight;
      if (!w2 || !h2) {
      ...
      const s2 = Math.min(maxEdge * 2, 2400) / Math.max(w2, h2);
      const rw = Math.max(1, Math.round(w2 * s2)), rh = Math.max(1, Math.round(h2 * s2));
      const canvas = document.createElement("canvas");
      canvas.width = rw;
      canvas.height = rh;
  ...
  async function preview(blob, key, bgHex = "#FFFFFF", maxEdge = 1e3) {
  ...
  async function previewPNG(blob, key, maxEdge = 700) {
  ...
  async function thumbUrl(blob, name = "") {
  ...
  async function buf(file) {
    if (!_buf.has(file)) _buf.set(file, await fetch(`fonts/${file}`).then((r2) => r2.arrayBuffer()));
  ...
  async function makeFonts(pdf) {
    ...
    for (const fam of ["DMSerifDisplay", "Inter", "JetBrainsMono"]) {
      const f2 = FONT_FILES[fam];
      try {
        const [rb, sb, bb] = await Promise.all([buf(f2.regular), buf(f2.semibold), buf(f2.bold)]);
        emb[fam] = {
        ...
        console.error("Missing font file for", fam);
      }
    }
  ...
  function hex(c2) {
    const v2 = c2.replace("#", "");
    const n2 = v2.length === 3 ? v2.split("").map((x2) => x2 + x2).join("") : v2;
    return rgb(parseInt(n2.slice(0, 2), 16) / 255, parseInt(n2.slice(2, 4), 16) / 255, parseInt(n2.slice(4, 6), 16) / 255);
  }
  function mix(c2, alpha) {
    const k2 = hex(c2);
    return rgb(k2.red + (1 - k2.red) * (1 - alpha), k2.green + (1 - k2.green) * (1 - alpha), k2.blue + (1 - k2.blue) * (1 - alpha));
  }
  function text(page, x2, y2, s2, font, size, color, { align = "left", tracking = 0, rotate = 0 } = {}) {
    let w2 = font.widthOfTextAtSize(s2, size);
    if (tracking) w2 += tracking * Math.max(s2.length - 1, 0);
    let xpt = x2 * MM;
    if (align === "center") xpt -= w2 / 2;
    else if (align === "right") xpt -= w2;
    const opts = { x: xpt, y: yT(y2), size, font, color, ...tracking ? { characterSpacing: tracking } : {} };
    if (rotate !== 0) {
      opts.rotate = degrees(rotate);
      if (rotate === -90) {
        if (align === "left") opts.y = yT(y2) - w2;
        else if (align === "right") opts.y = yT(y2);
        else if (align === "center") opts.y = yT(y2) - w2 / 2;
        opts.x = x2 * MM;
      }
    }
    page.drawText(s2, opts);
  }
  function rect(page, x2, y2, w2, h2, { fill, stroke, line = 0.5 } = {}) {
    page.drawRectangle({
      x: x2 * MM,
      y: yT(y2 + h2),
      width: w2 * MM,
  ...
  function hline(page, x1, x2, y2, color, w2 = 0.4) {
    page.drawLine({ start: { x: x1 * MM, y: yT(y2) }, end: { x: x2 * MM, y: yT(y2) }, thickness: w2, color });
  }
  function fit(img, bw, bh, pad) {
    const s2 = Math.min((bw - 2 * pad) / img.width, (bh - 2 * pad) / img.height);
    return { w: img.width * s2, h: img.height * s2 };
  }
  function clip(s2, font, size, maxMM) {
    const max = maxMM * MM;
    if (font.widthOfTextAtSize(s2, size) <= max) return s2;
    while (s2.length && font.widthOfTextAtSize(s2 + "\u2026", size) > max) s2 = s2.slice(0, -1);
    return s2 + "\u2026";
  }
  function wrapText(s2, font, size, maxMM) {
  ...
  function microLabel(page, x2, y2, s2, F2, size, color, { align = "left", track = size * 0.6, rotate = 0 } = {}) {
    text(page, x2, y2, caps(s2), F2("mono", "semibold"), size, color, { align, tracking: track, rotate });
  }
  function folio(page, x2, y2, no, total, F2, color, align = "right") {
    text(
      page,
      x2,
      y2,
  ...
  function drawTile(page, x2, y2, w2, h2, img, entry, F2, idx, numberLabel) {
    const bgFill = hex(P2.pastels[idx % P2.pastels.length]);
    rect(page, x2, y2, w2, h2, { fill: bgFill, stroke: hex(P2.ink), line: 0.25 });
    const pad = w2 * 0.15;
    const f2 = fit(img, w2, h2, pad);
    page.drawImage(img, { x: (x2 + w2 / 2) * MM - f2.w * MM / 2, y: yT(y2 + h2 / 2) - f2.h * MM / 2, width: f2.w * MM, height: f2.h * MM });
    const nFont = F2("body", "bold");
    const ty = y2 + h2 + 6;
    let tx = x2;
    if (numberLabel !== null) {
      text(page, tx, ty, String(numberLabel).padStart(2, "0"), F2("mono", "semibold"), 7, hex(P2.ink), { tracking: 0.5 });
      tx += 8;
    }
    text(page, tx, ty, clip(entry.name, nFont, 10, w2 - (tx - x2)), nFont, 10, hex(P2.ink), { tracking: displayTrack(10) });
    const meta = [typeLabel(entry.types), indLabel(entry)].filter(Boolean).join("    \u2014    ");
    if (meta) {
      microLabel(page, tx, ty + 5, meta, F2, 5.5, hex(P2.slate), { track: 1.5 });
    }
  }
  async function embedPreviews(pdf, src, profile, entries) {
    const prepared = await Promise.all(entries.map(async (e2) => {
      const blob = await src.getBlob(profile, `logos/${e2.file}`);
      const pv = await previewPNG(blob, `${profile}/${e2.file}`, 1e3);
      return { entry: e2, bytes: pv.bytes };
    }));
    const out = [];
    for (const p2 of prepared) out.push({ entry: p2.entry, img: await pdf.embedPng(p2.bytes) });
    return out;
  }
  async function loadBrandmark(pdf, theme) {
    ...
    for (const ext of ["svg", "png", "jpg"]) {
      try {
        ...
        if (!pv) {
          pv = await previewPNG(await res.blob(), `brand/${ck}`, 700);
          _markCache.set(ck, pv);
        }
  ...
  function drawMark(page, mark, xMM, topMM, heightMM, align = "left") {
    ...
    const x2 = align === "center" ? xMM - wMM / 2 : align === "right" ? xMM - wMM : xMM;
    page.drawImage(mark, { x: x2 * MM, y: yT(topMM + heightMM), width: wMM * MM, height: heightMM * MM });
    return wMM;
  }
  function drawCover(page, theme, F2, ctx) {
    rect(page, 0, 0, 297, 210, { fill: hex(P2.canvas) });
    const fg = hex(P2.ink), accent = hex(P2.yellow);
    ...
    microLabel(page, 24 + 12, 84, kicker, F2, 8, fg, { track: 3 });
    const dFont = F2("display", "regular");
    ...
    text(page, 24, ty - 20, sub, F2("body", "regular"), 16, hex(P2.slate), { tracking: -0.16 });
    if (ctx.clientName) {
      microLabel(page, 297 - 24, 165, "Prepared for", F2, 7, fg, { align: "right", track: 2 });
      text(page, 297 - 24, 173, ctx.clientName, F2("body", "semibold"), 14, fg, { align: "right", tracking: -0.14 });
      if (ctx.dateStr) microLabel(page, 297 - 24, 182, ctx.dateStr, F2, 7, hex(P2.slate), { align: "right", track: 1.5 });
    }
    microLabel(page, 297 - 24, 24, `${ctx.n} selected`, F2, 7, hex(P2.slate), { align: "right", track: 2.5 });
  }
  function runFooter(page, theme, F2, pageNo, total, subtitle) {
    hline(page, 24, 297 - 24, 195, mix(P2.ink, 0.1), 0.25);
    text(page, 24, 201, theme.name, F2("body", "semibold"), 8, hex(P2.ink), { tracking: 0.5 });
    microLabel(page, 297 / 2, 201, subtitle, F2, 6, hex(P2.slate), { align: "center", track: 2.5 });
    folio(page, 297 - 24, 201, pageNo, total, F2, hex(P2.ink));
  }
  function drawClosing(page, theme, F2, ctx) {
    rect(page, 0, 0, 297, 210, { fill: hex(P2.canvas) });
    const fg = hex(P2.ink), accent = hex(P2.yellow);
    rect(page, 297 / 2 - 3, 100 - 6, 6, 6, { fill: accent });
    microLabel(page, 297 / 2, 110, "End of Document", F2, 8, fg, { align: "center", track: 3 });
    text(page, 297 / 2, 126, theme.name, F2("display", "regular"), 32, fg, { align: "center", tracking: displayTrack(32) });
  }
  function renderBentoPage(page, theme, F2, bTiles, startNo) {
    const top = 22, gap = 12, bw = 118.5, sw = 53.25, bh = 76, sh = 46;
    const coords = [
      { x: 24, y: top, w: bw, h: bh },
      // L-Top (Wide)
      { x: 24, y: top + bh + 16 + gap, w: sw, h: sh },
      // L-Bot1 (Sq)
      { x: 24 + sw + gap, y: top + bh + 16 + gap, w: sw, h: sh },
      // L-Bot2 (Sq)
      { x: 24 + bw + gap, y: top, w: sw, h: sh },
      // R-Top1 (Sq)
      { x: 24 + bw + gap + sw + gap, y: top, w: sw, h: sh },
      // R-Top2 (Sq)
      { x: 24 + bw + gap, y: top + sh + 16 + gap, w: bw, h: bh }
      // R-Bot (Wide)
    ];
    bTiles.forEach((t2, i2) => {
      if (i2 < 6) drawTile(page, coords[i2].x, coords[i2].y, coords[i2].w, coords[i2].h, t2.img, t2.entry, F2, i2, startNo + i2 + 1);
    });
  }
  function renderShowcasePage(page, theme, F2, bTiles, startNo) {
    const cols = 3, top = 24, gm = 24, gapX = 12, gapY = 24;
    const tileW = 75, tileH = 55, capH = 16;
    bTiles.forEach((t2, i2) => {
      if (i2 < 6) {
        const r2 = Math.floor(i2 / cols), c2 = i2 % cols;
        const x2 = gm + c2 * (tileW + gapX);
        const y2 = top + r2 * (tileH + capH + gapY);
        drawTile(page, x2, y2, tileW, tileH, t2.img, t2.entry, F2, i2, startNo + i2 + 1);
      }
    });
  }
  function renderLookbookPage(page, theme, F2, bTiles, startNo) {
    const top = 32, gm = 24, gap = 12;
    const bw = 118.5, bh = 118.5;
    const capH = 16;
    const sh = (bh - capH - gap) / 2;
    const sw = bw;
    const coords = [
    ...
    bTiles.forEach((t2, i2) => {
      if (i2 < 3) drawTile(page, coords[i2].x, coords[i2].y, coords[i2].w, coords[i2].h, t2.img, t2.entry, F2, i2, startNo + i2 + 1);
    });
  }
  function renderHeroPage(page, theme, F2, bTiles, startNo) {
    const w2 = 180, h2 = 120, cx = 297 / 2, cy = 190 / 2;
    if (bTiles[0]) {
      drawTile(page, cx - w2 / 2, cy - h2 / 2, w2, h2, bTiles[0].img, bTiles[0].entry, F2, 0, startNo + 1);
    }
  }
  function drawSmartContent(pdf, theme, F2, tiles) {
    const pages = [];
    let startNo = 0;
    let wList = tiles.filter((t2) => t2.img.width / t2.img.height >= 1.25);
    let sList = tiles.filter((t2) => t2.img.width / t2.img.height < 1.25);
    while (wList.length > 0 || sList.length > 0) {
      if (wList.length >= 2 && sList.length >= 4) {
        const p2 = pdf.addPage([PAGE_W, PAGE_H]);
        p2._F = F2;
        pages.push(p2);
        rect(p2, 0, 0, 297, 210, { fill: hex(P2.canvas) });
        const bTiles = [wList.shift(), sList.shift(), sList.shift(), sList.shift(), sList.shift(), wList.shift()];
        renderBentoPage(p2, theme, F2, bTiles, startNo);
        startNo += 6;
        ...
        renderLookbookPage(p2, theme, F2, bTiles, startNo);
        startNo += 3;
        ...
        renderShowcasePage(p2, theme, F2, bTiles, startNo);
        startNo += 6;
        ...
        renderHeroPage(p2, theme, F2, bTiles, startNo);
        startNo += 1;
      } else {
        const p2 = pdf.addPage([PAGE_W, PAGE_H]);
        p2._F = F2;
  ...
  async function renderSlice(src, profile, theme, entries, query, opts = {}) {
    const pdf = await PDFDocument.create();
    pdf.setTitle(`${theme.name} \u2014 Logo slice`);
    pdf.setCreator("Logo Showcase \xB7 HaseebMadeIt");
    const T2 = theme;
    const F2 = await makeFonts(pdf);
    ...
    ctx.mark = await loadBrandmark(pdf, T2);
    if (includeCover) drawCover(pdf.addPage([PAGE_W, PAGE_H]), T2, F2, ctx);
    const contentPages = drawSmartContent(pdf, T2, F2, tiles);
    if (includeClosing) drawClosing(pdf.addPage([PAGE_W, PAGE_H]), T2, F2, ctx);
    const total = pdf.getPageCount();
    const startNo = includeCover ? 2 : 1;
    contentPages.forEach((pg, i2) => runFooter(pg, T2, F2, startNo + i2, total, subtitle));
    return pdf.save();
  }
  async function renderRange(src, profile, theme, entries, query, opts = {}) {
    const pdf = await PDFDocument.create();
    ...
    const ctx = { subtitle, n: entries.length, clientName: opts.clientName || "", dateStr: opts.dateStr || "" };
    ctx.mark = await loadBrandmark(pdf, T2);
    if (opts.includeCover !== false) drawCover(pdf.addPage([PAGE_W, PAGE_H]), T2, F2, ctx);
    const cols = 5;
    ...
    for (let i2 = 0; i2 < tiles.length; i2++) {
      ...
      const x2 = gm + col * (tileW + gap);
      drawTile(page, x2, rowY, tileW, imgH, img, entry, F2, i2, null);
      if (++col >= cols) {
        col = 0;
        rowY += tileH + rowGap;
      }
    }
    if (opts.includeClosing !== false) drawClosing(pdf.addPage([PAGE_W, PAGE_H]), T2, F2, ctx);
    const total = pdf.getPageCount();
    const startNo = opts.includeCover !== false ? 2 : 1;
    pages.forEach((pg, i2) => runFooter(pg, T2, F2, startNo + i2, total, subtitle));
    return pdf.save();
  }
  async function renderByType(src, profile, theme, groups, opts = {}) {
    const pdf = await PDFDocument.create();
    pdf.setTitle(`${theme.name} \u2014 by type`);
    pdf.setCreator("Logo Showcase \xB7 HaseebMadeIt");
    const T2 = theme;
    const F2 = await makeFonts(pdf);
    const all = groups.flatMap((g2) => g2.entries);
    const prepared = await embedPreviews(pdf, src, profile, all);
    const imgOf = new Map(prepared.map((p2) => [p2.entry.file, p2.img]));
    ...
    for (const g2 of groups) {
      if (y2 + 36 > 180) newPage();
      const shown = g2.entries.length, total2 = g2.total ?? shown;
      microLabel(page, gm, y2 + 4, g2.type.replace(/-/g, " "), F2, 8.5, hex(P2.ink), { track: 2.5 });
      text(
        page,
        297 - gm,
        y2 + 4,
        total2 === shown ? `${total2} mark${total2 !== 1 ? "s" : ""}` : `${shown} of ${total2}`,
        F2("mono", "semibold"),
        7,
        hex(P2.slate),
        { align: "right" }
      );
      hline(page, gm, 297 - gm, y2 + 8, mix(P2.ink, 0.2), 0.5);
      y2 += 16;
      let col = 0;
      for (let i2 = 0; i2 < g2.entries.length; i2++) {
        const e2 = g2.entries[i2];
        if (y2 + tileH > 180) {
          newPage();
          col = 0;
        }
        const x2 = gm + col * (tileW + gap);
        const img = imgOf.get(e2.file);
        if (img) {
          drawTile(page, x2, y2, tileW, imgH, img, e2, F2, i2, null);
        }
        if (++col >= cols) {
          col = 0;
          y2 += tileH + rowGap;
  ...
  var init_pdf = __esm({
    "web/src/pdf.js"() {
      init_curate();
      init_images();
      ({ PDFDocument, rgb, degrees } = window.PDFLib);
      MM = 72 / 25.4;
      PAGE_W = 297 * MM;
      PAGE_H = 210 * MM;
      P2 = {
      ...
      _buf = /* @__PURE__ */ new Map();
      yT = (mm) => PAGE_H - mm * MM;
      caps = (s2) => String(s2 ?? "").toUpperCase();
  ...
  var require_app = __commonJS({
    "web/app.js"() {
      ...
      var esc = (s2) => String(s2 ?? "").replace(/[&<>"']/g, (c2) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[c2]);
      var IMG_KINDS = /* @__PURE__ */ new Set(["image", "pdf"]);
      var state = {
        source: new DemoPortfolio(),
        isDemo: true,
        srcGen: 0,
        profileId: "eikon",
        profileName: "Eikon Design",
        theme: THEMES.eikon,
        types: [],
        pendingHandle: null,
        assetsCache: /* @__PURE__ */ new Map(),
        // typeKey -> [assets]
        blobs: /* @__PURE__ */ new Map(),
        // `${type}/${file}` -> objectURL
        logos: [],
        picked: [],
        // showcase selection (ordered file list)
        live: { bytes: null, url: null, running: false, pending: false, wantId: 0 },
        modalUrl: null
      };
      var mk = { mode: "slice", layout: "grid", columns: "", density: "", cover: true, closing: false, perType: "" };
      var filters = { industry: "", type: "" };
      async function blobUrl(typeKey, file) {
        const k2 = `${typeKey}/${file}`;
        if (state.blobs.has(k2)) return state.blobs.get(k2);
        ...
        state.blobs.set(k2, url);
        return url;
      }
      function clearBlobs() {
        state.blobs.forEach((u2) => URL.revokeObjectURL(u2));
        state.blobs.clear();
      }
      ...
      function toast(msg, kind = "ok") {
        ...
        $2("#toasts").appendChild(el);
        const kill = () => {
          el.classList.add("out");
          setTimeout(() => el.remove(), 220);
        };
        const t2 = setTimeout(kill, 3200);
        el.addEventListener("mouseenter", () => clearTimeout(t2));
        el.addEventListener("mouseleave", () => setTimeout(kill, 1e3));
      }
      async function onSourceChanged() {
        ...
        filters.industry = "";
        filters.type = "";
        const fi = $2("#filter-industry");
        if (fi) fi.value = "";
        const ft2 = $2("#filter-type");
        if (ft2) ft2.value = "";
        const ls = $2("#logos-search");
        if (ls) ls.value = "";
        const prof = state.source.profile();
        state.profileId = prof.id;
        state.theme = prof.theme || THEMES.eikon;
        state.profileName = state.theme.name || prof.id;
        state.types = await state.source.types();
        $2("#source-text").textContent = state.isDemo ? "Demo portfolio" : state.profileName;
        $2("#source-badge").classList.toggle("live", !state.isDemo);
        $2("#connect-banner").hidden = !state.isDemo || !fsAccessSupported();
        $2("#logos-title").textContent = `${state.profileName} \u2014 Logos`;
        renderSettings();
        buildTabs();
      ...
      async function useDemo() {
        state.source = new DemoPortfolio();
        state.isDemo = true;
        await state.source._manifest();
        await onSourceChanged();
      }
      async function useFolder(h2) {
        state.source = new FsPortfolio(h2);
        state.isDemo = false;
        await onSourceChanged();
      }
      var currentView = "logos";
      var currentType = null;
      function viewExists(v2) {
        if (v2 === "logos" || v2 === "settings") return true;
        if (v2 === "browse") return state.types.length > 0;
        if (v2.startsWith("assets:")) return state.types.some((t2) => t2.key === v2.slice(7));
        return false;
      }
      function buildTabs() {
        const tabs = $2("#tabs");
        ...
        const add = (label, view) => {
          ...
          b2.className = "tab";
          b2.dataset.view = view;
          b2.role = "tab";
          b2.innerHTML = `<span>${label}</span>`;
          b2.addEventListener("click", () => switchView(view));
          tabs.appendChild(b2);
        };
        if (state.types.some((t2) => t2.key === "logos")) add("Logos", "logos");
        if (state.types.length) add("Files", "browse");
        state.types.filter((t2) => t2.key !== "logos").forEach((t2) => add(t2.label, `assets:${t2.key}`));
        add("Settings", "settings");
      }
      function switchView(view) {
        currentView = view;
        $$(".tab").forEach((t2) => {
          const a2 = t2.dataset.view === view;
          t2.classList.toggle("active", a2);
          t2.setAttribute("aria-selected", String(a2));
        });
        const isAssets = view.startsWith("assets:");
        $2("#view-logos").hidden = view !== "logos";
        $2("#view-settings").hidden = view !== "settings";
        $2("#view-browse").hidden = view !== "browse";
        $2("#view-assets").hidden = !isAssets;
        if (isAssets) {
          currentType = view.slice(7);
          renderAssets(currentType);
        }
        if (view === "browse") renderBrowse();
      }
      function imageLogos() {
        return state.logos.filter((a2) => IMG_KINDS.has(a2.kind)).filter((a2) => !filters.industry || (a2.industries || []).includes(filters.industry)).filter((a2) => !filters.type || (a2.types || []).includes(filters.type));
      }
      var toEntry = (a2) => ({ file: a2.file, name: a2.brand, types: a2.types || [], industries: a2.industries || [], tier: 1, year: null });
      function logoEntries(files) {
        const byFile = new Map(state.logos.map((a2) => [a2.file, a2]));
        return files.map((f2) => byFile.get(f2)).filter(Boolean).map(toEntry);
      }
      ...
      async function renderPicker() {
        const box = $2("#logos-picker");
        const q2 = ($2("#logos-search").value || "").toLowerCase();
        const renderable = state.logos.filter((a2) => IMG_KINDS.has(a2.kind));
        const matched = imageLogos();
        const imgs = matched.filter((a2) => !q2 || a2.brand.toLowerCase().includes(q2));
        box.innerHTML = "";
        const empty = $2("#logos-empty");
        ...
        if (!matched.length) {
          ...
          const cf = $2("#logos-clearfilter");
          if (cf) cf.addEventListener("click", () => {
            filters.industry = "";
            filters.type = "";
            $2("#filter-industry").value = "";
            $2("#filter-type").value = "";
            renderPicker();
            scheduleRender(120);
          });
        ...
        for (const a2 of imgs) {
          const order = state.picked.indexOf(a2.file);
          const el = document.createElement("div");
          el.className = "pcard" + (order >= 0 ? " sel" : "");
          el.innerHTML = `<div class="pthumb"><img alt="${esc(a2.brand)}">${order >= 0 ? `<span class="pnum">${order + 1}</span>` : ""}</div><div class="pname">${esc(a2.brand)}</div>`;
          const im = el.querySelector("img");
          logoThumbUrl(a2).then((u2) => {
            if (im) im.src = u2;
          }).catch(() => {
          });
        ...
        const n2 = state.picked.length;
        const other = state.logos.length - renderable.length;
        $2("#logos-pickcount").textContent = (n2 ? `${n2} selected` : `${matched.length} logos`) + (other ? ` \xB7 ${other} not embeddable` : "");
      }
      async function logoThumbUrl(a2) {
        if (a2.kind === "pdf") return thumbUrl(await state.source.getBlob("logos", a2.file), a2.file);
        return blobUrl("logos", a2.file);
      }
      function togglePick(file) {
        const i2 = state.picked.indexOf(file);
        if (i2 >= 0) state.picked.splice(i2, 1);
        else state.picked.push(file);
        renderPicker();
        scheduleRender(120);
      }
      var logoSrcAdapter = { getBlob: (_p, path) => state.source.getBlob("logos", path.replace(/^logos\//, "")) };
      function setPreviewStatus(busy) {
        const s2 = $2("#preview-status");
        s2.classList.toggle("busy", busy);
        $2("#preview-status-text").textContent = busy ? "Updating\u2026" : "Up to date";
        $2("#preview-shell").classList.toggle("updating", busy);
      }
      ...
      function subtitleFor() {
        if (filters.industry && filters.type) return `${titleWord(filters.industry)} \xB7 ${titleWord(filters.type)}`;
      ...
      var titleWord = (s2) => s2.replace(/-/g, " ").replace(/\b\w/g, (c2) => c2.toUpperCase());
      async function buildCurrent() {
        const entries = pickedOrAll();
        if (!entries.length) throw new Error("No PNG/JPG logos match");
        const q2 = { industries: [], types: [], matchAll: false };
        const opts = {
          layout: mk.layout,
        ...
        if (mk.mode === "bytype") {
          ...
          for (const e2 of entries) {
            const t2 = e2.types && e2.types[0] || "untyped";
            if (!byType.has(t2)) byType.set(t2, []);
            byType.get(t2).push(e2);
          }
          const cap = mk.perType ? parseInt(mk.perType, 10) : 0;
          const groups = [...byType.entries()].sort((a2, b2) => (TYPES2.indexOf(a2[0]) + 99) % 100 - (TYPES2.indexOf(b2[0]) + 99) % 100).map(([type, es]) => ({ type, total: es.length, entries: cap ? es.slice(0, cap) : es }));
          const bytes2 = await renderByType(logoSrcAdapter, state.profileId, state.theme, groups, opts);
          return { bytes: bytes2, n: entries.length };
        }
        const bytes = mk.mode === "slice" ? await renderSlice(logoSrcAdapter, state.profileId, state.theme, entries, q2, opts) : await renderRange(logoSrcAdapter, state.profileId, state.theme, entries, q2, opts);
      ...
      async function runRender() {
        ...
        setPreviewStatus(true);
        $2("#preview-status").classList.remove("err");
        try {
          const r2 = await buildCurrent();
          if (myId !== state.live.wantId) return;
          if (state.live.url) URL.revokeObjectURL(state.live.url);
          state.live.bytes = r2.bytes;
          state.live.url = URL.createObjectURL(new Blob([r2.bytes], { type: "application/pdf" }));
          const frame = $2("#live-frame");
          frame.onload = null;
          frame.onload = () => {
            $2("#preview-skeleton").hidden = true;
            frame.hidden = false;
          ...
          if (myId === state.live.wantId) {
            setPreviewStatus(false);
            $2("#preview-status-text").textContent = e2.message;
            $2("#preview-status").classList.add("err");
          }
        } finally {
          state.live.running = false;
          if (state.live.pending || myId !== state.live.wantId) {
            state.live.pending = false;
            runRender();
          } else if (!$2("#preview-status").classList.contains("err")) setPreviewStatus(false);
        }
      }
      function setBusy(btn, busy) {
        if (!btn) return;
        btn.disabled = busy;
        const sp = btn.querySelector(".cta-spin");
        if (sp) sp.hidden = !busy;
      }
      function whenIdle(timeout = 6e3) {
        return new Promise((res) => {
          const s2 = Date.now();
          const t2 = setInterval(() => {
            if (!state.live.running && !state.live.pending || Date.now() - s2 > timeout) {
              clearInterval(t2);
              res();
            }
          }, 70);
      ...
      function deckFilename() {
        const p2 = [state.profileId];
        if (mk.mode === "bytype") p2.push("by-type");
        else if (filters.industry || filters.type) {
          if (filters.industry) p2.push(filters.industry);
          if (filters.type) p2.push(filters.type);
        } else p2.push(mk.mode === "range" ? "all" : "selected");
        return p2.join("-") + ".pdf";
      }
      async function downloadLive() {
        const btn = $2("#preview-download2");
        ...
        const lab = btn.querySelector(".cta-label");
      ...
      async function renderAssets(typeKey) {
        const t2 = TYPES.find((x2) => x2.key === typeKey);
        $2("#assets-title").textContent = `${state.profileName} \u2014 ${t2.label}`;
        $2("#assets-lede").textContent = "Search a brand, preview, then download to share with the client.";
        const grid = $2("#assets-grid");
        grid.innerHTML = "<div class='muted' style='padding:1rem'>Loading\u2026</div>";
        let assets = state.assetsCache.get(typeKey);
        if (!assets) {
          assets = await state.source.assets(typeKey).catch(() => []);
          state.assetsCache.set(typeKey, assets);
        }
        const q2 = ($2("#assets-search").value || "").toLowerCase();
        const visible = assets.filter((a2) => !q2 || a2.brand.toLowerCase().includes(q2));
        grid.innerHTML = "";
        $2("#assets-empty").hidden = visible.length !== 0;
        $2("#assets-empty-t").textContent = assets.length ? "No brands match your search" : `No ${t2.label} in this portfolio yet`;
        for (const a2 of visible) grid.appendChild(assetCard(typeKey, a2));
      }
      function assetCard(typeKey, a2) {
        const card = document.createElement("div");
        card.className = "acard";
        const badge = a2.kind === "pdf" ? "PDF" : a2.kind === "video" ? "VIDEO" : a2.ext.toUpperCase();
        const sub = a2.industryLabel ? esc(a2.industryLabel) : a2.descriptor ? esc(a2.descriptor) : "";
        card.innerHTML = `<div class="athumb ${a2.kind}"><span class="abadge">${esc(badge)}</span></div>
    <div class="acap"><span class="anm">${esc(a2.brand)}</span>
      <span class="acap-row"><span class="asub">${sub}</span><span class="aview">View \u203A</span></span></div>`;
        const thumb = card.querySelector(".athumb");
        if (a2.kind === "image" || a2.kind === "pdf") {
          const im = document.createElement("img");
          const p2 = a2.kind === "pdf" ? state.source.getBlob(typeKey, a2.file).then((b2) => thumbUrl(b2, a2.file)) : blobUrl(typeKey, a2.file);
          Promise.resolve(p2).then((u2) => {
            im.src = u2;
          }).catch(() => {
          });
          thumb.appendChild(im);
        }
        card.addEventListener("click", () => openAsset(typeKey, a2));
        return card;
      }
      var browseOpen = /* @__PURE__ */ new Set();
      var browseInit = false;
      async function loadAssets(typeKey) {
        let a2 = state.assetsCache.get(typeKey);
        if (!a2) {
          a2 = await state.source.assets(typeKey).catch(() => []);
          state.assetsCache.set(typeKey, a2);
        }
        return a2;
      }
      async function renderBrowse() {
        const tree = $2("#browse-tree");
        const q2 = ($2("#browse-search").value || "").trim().toLowerCase();
        tree.innerHTML = "<div class='muted' style='padding:1rem'>Loading\u2026</div>";
        const data = [];
        for (const t2 of state.types) {
          const assets = await loadAssets(t2.key);
          const visible = assets.filter((a2) => !q2 || a2.brand.toLowerCase().includes(q2) || a2.file.toLowerCase().includes(q2) || (a2.industryLabel || "").toLowerCase().includes(q2));
          data.push({ t: t2, total: assets.length, visible });
        }
        tree.innerHTML = "";
        const totalFiles = data.reduce((n2, d2) => n2 + d2.total, 0);
        const matchFiles = data.reduce((n2, d2) => n2 + d2.visible.length, 0);
        const emptyT = $2("#browse-empty-t");
        $2("#browse-empty").hidden = !(totalFiles === 0 || matchFiles === 0);
        if (totalFiles === 0) {
          emptyT.textContent = "No files in this portfolio yet";
          return;
        }
        if (matchFiles === 0) {
          emptyT.textContent = "No files match your search";
          return;
        }
        for (const { t: t2, total, visible } of data) {
          if (q2 && !visible.length) continue;
          const open = q2 ? true : browseOpen.has(t2.key);
          const node = document.createElement("div");
          node.className = "tnode" + (open ? " open" : "");
          const count = q2 ? `${visible.length} of ${total}` : `${total}`;
          node.innerHTML = `<button class="tfhead" type="button" aria-expanded="${open}">
        <span class="tchev">\u25B8</span><span class="tfico">\u{1F4C1}</span>
        <span class="tflabel">${esc(t2.label)}</span><span class="tcount">${esc(count)}</span></button>
      <div class="tfiles"><div class="assetgrid tgrid"></div></div>`;
          const grid = node.querySelector(".tgrid");
          const fill = () => {
            if (grid.childElementCount) return;
            for (const a2 of visible) grid.appendChild(assetCard(t2.key, a2));
          };
          if (open) fill();
          if (!q2) node.querySelector(".tfhead").addEventListener("click", () => {
            const o2 = browseOpen.has(t2.key);
            if (o2) browseOpen.delete(t2.key);
            else {
              browseOpen.add(t2.key);
              fill();
            }
            node.classList.toggle("open", !o2);
            node.querySelector(".tfhead").setAttribute("aria-expanded", String(!o2));
          });
          tree.appendChild(node);
        }
      }
      async function openAsset(typeKey, a2) {
        const url = await blobUrl(typeKey, a2.file);
        $2("#preview-title").textContent = a2.brand;
        $2("#preview-sub").textContent = `${TYPES.find((x2) => x2.key === typeKey).label} \xB7 ${a2.ext.toUpperCase()}`;
        const frame = $2("#preview-frame"), img = $2("#preview-img"), vid = $2("#preview-video"), load = $2("#preview-loading");
        frame.hidden = img.hidden = vid.hidden = true;
        load.hidden = false;
        ...
        if (a2.kind === "pdf") {
          frame.hidden = false;
          frame.onload = () => load.hidden = true;
          frame.src = url;
          setTimeout(() => load.hidden = true, 1200);
        } else if (a2.kind === "video") {
          vid.src = url;
          vid.hidden = false;
          load.hidden = true;
        } else {
          ...
          img.src = url;
          img.hidden = false;
          setTimeout(() => load.hidden = true, 800);
        }
        $2("#preview").hidden = false;
      }
      function closeModal() {
        $2("#preview").hidden = true;
        $2("#preview-frame").src = "about:blank";
        $2("#preview-video").removeAttribute("src");
        $2("#preview-img").removeAttribute("src");
        if (state.modalUrl) {
      ...
      function expandLive() {
        ...
        $2("#preview-title").textContent = `${state.profileName} \u2014 Logos`;
        $2("#preview-sub").textContent = "Live preview";
        const frame = $2("#preview-frame"), img = $2("#preview-img"), vid = $2("#preview-video"), load = $2("#preview-loading");
        img.hidden = vid.hidden = true;
        frame.hidden = false;
        load.hidden = false;
        frame.onload = () => load.hidden = true;
        frame.src = state.modalUrl + "#toolbar=0&navpanes=0&view=FitH";
        setTimeout(() => load.hidden = true, 1e3);
        $2("#preview").hidden = false;
      }
      function renderSettings() {
        $2("#fs-supported").hidden = !fsAccessSupported();
        $2("#fs-unsupported").hidden = fsAccessSupported();
        $2("#settings-source").textContent = state.isDemo ? "Showing the built-in demo portfolio." : `Linked: ${state.profileName}`;
        $2("#disconnect-folder").hidden = state.isDemo;
      }
      async function connectFolder() {
        try {
          const h2 = await window.showDirectoryPicker({ mode: "read" });
          if (!await verifyPermission(h2)) return;
          state.pendingHandle = null;
          await saveHandle(h2);
          await useFolder(h2);
          toast(`Linked ${state.profileName}`);
        } catch (e2) {
          if (e2.name !== "AbortError") toast("Couldn't open that folder", "err");
        }
      }
      ...
      async function reconnectFolder() {
        const h2 = state.pendingHandle;
        if (!h2) return connectFolder();
        if (await verifyPermission(h2)) {
          state.pendingHandle = null;
          await saveHandle(h2);
          await useFolder(h2);
          toast(`Reconnected ${state.profileName}`);
        }
      }
      function showReconnectBanner() {
        $2("#banner-text").innerHTML = `Portfolio <strong>${esc(state.pendingHandle?.name || "")}</strong> is saved \u2014 reconnect to continue.`;
        $2("#banner-connect").textContent = "Reconnect \u2192";
        $2("#connect-banner").hidden = false;
      }
      function applyTheme(t2) {
        document.documentElement.dataset.theme = t2;
        $2("#theme-toggle").textContent = t2 === "dark" ? "\u2600" : "\u263E";
        localStorage.setItem("ls-theme", t2);
      }
      function initTheme() {
      ...
      function buildPalette() {
        PALETTE = [
          ...
          ...state.types.length ? [{ label: "Files \xB7 browse folders", run: () => switchView("browse") }] : [],
          ...state.types.filter((t2) => t2.key !== "logos").map((t2) => ({ label: `Open \xB7 ${t2.label}`, run: () => switchView(`assets:${t2.key}`) })),
          { label: "Settings", run: () => switchView("settings") },
          { label: "Output \xB7 Slice", run: () => {
            setSeg($2("#opt-mode"), "slice");
            mk.mode = "slice";
            syncMode();
            scheduleRender(60);
          } },
          { label: "Output \xB7 Range sheet", run: () => {
            setSeg($2("#opt-mode"), "range");
            mk.mode = "range";
            syncMode();
            scheduleRender(60);
          } },
      ...
      function openPalette() {
        $2("#palette").hidden = false;
        $2("#palette-input").value = "";
        palIdx = 0;
        renderPalette("");
        setTimeout(() => $2("#palette-input").focus(), 30);
      }
      var closePalette = () => {
        $2("#palette").hidden = true;
      };
      function renderPalette(q2) {
        const list = PALETTE.filter((c2) => c2.label.toLowerCase().includes(q2.toLowerCase()));
        const box = $2("#palette-list");
        box.innerHTML = "";
        list.forEach((c2, i2) => {
          const el = document.createElement("div");
          el.className = "palette-item" + (i2 === palIdx ? " active" : "");
          el.innerHTML = `<span>${c2.label}</span>${c2.key ? `<span class="pk">${c2.key}</span>` : ""}`;
          el.onclick = () => {
            closePalette();
            c2.run();
          };
      ...
      function setSeg(el, val) {
        el.querySelectorAll(".seg").forEach((b2) => {
          const a2 = b2.dataset.val === val;
          b2.classList.toggle("active", a2);
          b2.setAttribute("aria-checked", String(a2));
        });
      }
      function wireSeg(el, cb) {
        el.querySelectorAll(".seg").forEach((b2) => b2.addEventListener("click", () => {
          setSeg(el, b2.dataset.val);
          cb(b2.dataset.val);
        }));
      }
      function syncMode() {
        const lay = $2("#opt-layout");
        const dim = mk.mode !== "slice";
        lay.style.opacity = dim ? ".4" : "1";
        lay.style.pointerEvents = dim ? "none" : "";
        $2("#pertype-block").style.display = mk.mode === "bytype" ? "" : "none";
      }
      function syncLayout() {
        const f2 = $2("#opt-cols-field");
        const lb = mk.layout === "lookbook";
        f2.style.opacity = lb ? ".4" : "1";
        f2.style.pointerEvents = lb ? "none" : "";
      }
      function wire() {
        $2("#logos-search").addEventListener("input", renderPicker);
        $2("#logos-all").addEventListener("click", () => {
          state.picked = imageLogos().map((a2) => a2.file);
          renderPicker();
          scheduleRender(120);
        });
        $2("#logos-none").addEventListener("click", () => {
          state.picked = [];
          renderPicker();
          scheduleRender(120);
        });
        wireSeg($2("#opt-mode"), (v2) => {
          mk.mode = v2;
          syncMode();
          scheduleRender(60);
        });
        wireSeg($2("#opt-layout"), (v2) => {
          mk.layout = v2;
          syncLayout();
          scheduleRender(60);
        });
        wireSeg($2("#opt-density"), (v2) => {
          mk.density = v2;
          scheduleRender(60);
        });
        $2("#opt-cols").addEventListener("change", (e2) => {
          mk.columns = e2.target.value;
          scheduleRender(60);
        });
        $2("#opt-cover").addEventListener("change", (e2) => {
          mk.cover = e2.target.checked;
          scheduleRender(60);
        });
        $2("#opt-closing").addEventListener("change", (e2) => {
          mk.closing = e2.target.checked;
          scheduleRender(60);
        });
        $2("#opt-pertype").addEventListener("change", (e2) => {
          mk.perType = e2.target.value;
          scheduleRender(60);
        });
        $2("#preview-download2").addEventListener("click", downloadLive);
        $2("#preview-expand").addEventListener("click", expandLive);
        $2("#industry-list").innerHTML = INDUSTRIES.map((v2) => `<option value="${v2}">`).join("");
        $2("#filter-type").innerHTML = `<option value="">All types</option>` + TYPES2.map((v2) => `<option value="${v2}">${v2}</option>`).join("");
        $2("#filter-industry").addEventListener("input", (e2) => {
          filters.industry = e2.target.value.trim().toLowerCase().replace(/\s+/g, "-");
          state.picked = [];
          renderPicker();
          scheduleRender(120);
        });
        $2("#filter-type").addEventListener("change", (e2) => {
          filters.type = e2.target.value;
          state.picked = [];
          renderPicker();
          scheduleRender(120);
        });
        $$(".cta").forEach((c2) => c2.addEventListener("mousemove", (e2) => {
          const r2 = c2.getBoundingClientRect();
          c2.style.setProperty("--x", `${e2.clientX - r2.left}px`);
          c2.style.setProperty("--y", `${e2.clientY - r2.top}px`);
        }));
        $2("#assets-search").addEventListener("input", () => currentType && renderAssets(currentType));
        $2("#browse-search").addEventListener("input", renderBrowse);
        $2("#connect-folder").addEventListener("click", connectFolder);
        $2("#banner-connect").addEventListener("click", reconnectFolder);
        $2("#disconnect-folder").addEventListener("click", disconnectFolder);
        $2("#theme-toggle").addEventListener("click", () => applyTheme(document.documentElement.dataset.theme === "dark" ? "light" : "dark"));
        $2("#open-palette").addEventListener("click", openPalette);
        $2("#palette-input").addEventListener("input", (e2) => {
          palIdx = 0;
          renderPalette(e2.target.value);
        });
        $$("[data-close]").forEach((b2) => b2.addEventListener("click", closeModal));
        $$("[data-palette-close]").forEach((b2) => b2.addEventListener("click", closePalette));
      ...
      function onKey(e2) {
        ...
        if (meta && e2.key.toLowerCase() === "k") {
          e2.preventDefault();
          return $2("#palette").hidden ? openPalette() : closePalette();
        }
        if (!$2("#palette").hidden) {
          const list = $2("#palette-list")._list || [];
          if (e2.key === "ArrowDown") {
            e2.preventDefault();
            palIdx = Math.min(palIdx + 1, list.length - 1);
            renderPalette($2("#palette-input").value);
            ...
            e2.preventDefault();
            const c2 = list[palIdx];
            if (c2) {
              closePalette();
              c2.run();
            }
          } else if (e2.key === "Escape") closePalette();
          return;
        }
        if (e2.key === "Escape" && !$2("#preview").hidden) return closeModal();
        if (meta && e2.key === "Enter") {
          e2.preventDefault();
          return downloadLive();
        }
        if (meta && e2.key.toLowerCase() === "d") {
          e2.preventDefault();
          return applyTheme(document.documentElement.dataset.theme === "dark" ? "light" : "dark");
        }
        if (typing) return;
        if (e2.key === "/") {
          const s2 = currentView === "browse" ? $2("#browse-search") : currentView.startsWith("assets:") ? $2("#assets-search") : $2("#logos-search");
          if (s2) {
            e2.preventDefault();
            s2.focus();
          }
      ...
      async function init() {
        ...
        if (fsAccessSupported()) {
          try {
            const h2 = await loadHandle();
            if (h2) {
              const perm = await h2.queryPermission({ mode: "read" });
              if (perm === "granted") {
                await useFolder(h2);
                return;
              }
              state.pendingHandle = h2;
              await useDemo();
              showReconnectBanner();
              return;
            }
